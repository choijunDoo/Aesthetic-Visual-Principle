...
class PDataset(data.Dataset):
    def __init__(self, csv_file, root_dir, transform=None):
        self.information = pd.read_csv(csv_file)
        self.root_dir = root_dir
        self.transform = transform
    ...
    def __getitem__(self, idx):
        img_name = os.path.join(self.root_dir, str(self.information.loc[idx, 'img']))
        image = Image.open(img_name).convert('RGB')
        information = self.information.iloc[idx, 1:].to_numpy()
        information = information.astype('float')
        sample = {'img_id': img_name, 'image': image, 'information': information}
        if self.transform:
            sample['image'] = self.transform(sample['image'])
