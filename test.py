...
for i, img in enumerate(test_imgs):
    ...
    for k, e in enumerate(out, 1):
        std += e * (k - mean) ** 2
    std = std ** 0.5
    gt = test_df[test_df.iloc[:,1] == img].to_numpy()[:, 2:12].reshape(10, 1)
    gt_mean = 0.0
...
correct = (np.array(preds) == np.array(gt_binary))
acc = correct.sum() / correct.size
print('Binary accuracy: {}'.format(acc))